...
import { closeFilterModal } from "../store/uiSlice";
import {
  setSelectedSoftware,
  setSelectedTag,
  setSearchInput,
...
async function fetchPopularPosts() {
  ...
  const sortedPosts = data.posts.sort(
    (a: PostItemData, b: PostItemData) => b.likesCount - a.likesCount,
  );
...
export default function CommunityPage() {
  ...
  const isFilterModalOpen = useSelector(
    (state: RootState) => state.ui.isFilterModalOpen,
  );
  ...
  const getTagsForSoftware = (software: string) => {
    if (software === "CLO")
      return ["Avatar", "Colorway", "Pattern", "Rendering"];
  ...
  useEffect(() => {
    if (router.isReady && !selectedPost) {
      ...
      if (queryTags === "MavelousDesigner")
        setSelectedSoftware("MarvelousDesigner");
  ...
  useEffect(() => {
    ...
    if (selectedSoftware === "MarvelousDesigner")
      params.set("tags", "MavelousDesigner");
  ...
  return (
    <DarkBackground>
      ...
      <MobileDrawer
        ...
        setIsAppExpanded={setIsAppExpanded}
        isFamilyExpanded={isFamilyExpanded}
        setIsFamilyExpanded={setIsFamilyExpanded}
        ...
        <FilterModalBody>
          <FilterBreadcrumb>
            ...
            <span className="active">
              {currentTitle === "전체 게시판" ? "전체" : currentTitle}
            </span>
          </FilterBreadcrumb>
      ...
      <Carousel banners={banners} onPostClick={handlePostClick} />
