...
interface MainPanelProps {
  ...
  setIsSoftwareOpen: (val: boolean) => void;
  setSelectedSoftware: (val: string) => void;
  ...
  setActiveKeyword: (val: string) => void;
...
export default function MainPanel({
  currentTitle, totalPostCount, canScrollLeft, canScrollRight, scrollTags, tagsScrollRef, checkTagsScroll,
  softwareRef, selectedSoftware, isSoftwareOpen, setIsSoftwareOpen, setSelectedSoftware, getTagsForSoftware,
  selectedTag, setSelectedTag, handleTagClick, searchInput, setSearchInput, activeKeyword, setActiveKeyword,
  handleSearchKeyDown, searchInputRef, setIsFilterModalOpen, sortRef, sortBy, setSortBy, isSortOpen,
  setIsSortOpen, sortOptions, pinnedPosts, currentPinnedIndex, handlePrevPin, handleNextPin, handlePostClick,
  getCategoryBadgeStyle, getCategoryName, status, postData, formatTimeAgo, loadMoreRef, isFetchingNextPage
}: MainPanelProps) {
  return (
    <MainPanelWrapper>
      <TopFilterBar>
          ...
          <TopFilterScrollContainer ref={tagsScrollRef} onScroll={checkTagsScroll}>
            ...
            <TagList>
              {getTagsForSoftware(selectedSoftware).map(tag => (
                <TagBtn key={tag} active={selectedTag === tag} onClick={() => handleTagClick(tag)}>{tag}</TagBtn>
              ))}
            </TagList>
        ...
        <SearchAreaMobileWrapper>
          <SearchInputBox hasValue={searchInput.length > 0}>
            ...
            {searchInput.length > 0 && (
              <ClearInputBtn onClick={() => { setSearchInput(''); setActiveKeyword(''); setSelectedTag(''); }}>
